...
```{r}
best_exhaustive_model_1_test = lm(median_house_value ~ longitude + latitude + I(ocean_proximity == "NEAR OCEAN") + longitude:housing_median_age + longitude:I(ocean_proximity == "INLAND") + longitude:median_income + longitude:I(ocean_proximity == "NEAR BAY") + latitude:housing_median_age + housing_median_age:population + housing_median_age:I(ocean_proximity == "INLAND") + population:median_income + median_income:I(ocean_proximity == "NEAR BAY"), data = housing_trn_data)
summary(best_exhaustive_model_1_test)$"adj.r.squared"
#names(coef(best_exhaustive_model_1))[-1]
```
...
```{r}
best_exhaustive_model_2_test = lm(median_house_value ~ housing_median_age + I(ocean_proximity == "INLAND") + I(ocean_proximity == "NEAR OCEAN") + longitude:total_rooms + longitude:I(ocean_proximity == "INLAND") + latitude:housing_median_age + latitude:population + housing_median_age:population + housing_median_age:I(ocean_proximity == "INLAND") + housing_median_age:I(ocean_proximity == "NEAR BAY"), data = housing_trn_data)
summary(best_exhaustive_model_2_test)$"adj.r.squared"
#names(coef(best_exhaustive_model_2))[-1]
```
...
```{r}
best_exhaustive_model_3_test = lm(median_house_value ~ median_income + I(ocean_proximity == "INLAND") + I(ocean_proximity == "NEAR OCEAN") + longitude:I(ocean_proximity == "INLAND") + latitude:housing_median_age + latitude:I(ocean_proximity == "NEAR BAY") + housing_median_age:population + housing_median_age:I(ocean_proximity == "INLAND") + housing_median_age:I(ocean_proximity == "NEAR BAY"), data = housing_trn_data)
summary(best_exhaustive_model_3_test)$"adj.r.squared"
#names(coef(best_exhaustive_model_3))[-1]
```
...
Doing so, we can see that we are closer to normal distribution when taking the log of `median_house_value`. However, we are still rejecting both null hypothesis's. We will still consider this model to be best used for prediction rather than inference due to assumptions not holding.
...
The `back_twoway_mod_finish_aic` model has the lowest loocv_rmse. We believe this will be the best model due to having the lowest AIC as well as Leave-One-Out Cross-Validation RMSE. 
...
TODO: We should only be predicting on our best model. Let's discuss this.
...
Now that we have decided our best model to be `back_twoway_mod_finish_aic`, we will evaluate the model on our test set and find the Root-Mean-Square-Error.
