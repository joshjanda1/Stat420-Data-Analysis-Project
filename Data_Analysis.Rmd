...
best_adjr2 = fit_all_sum$adjr2[which.max(fit_all_sum$adjr2)] #<- ?? we use this variable three times to give adj r2

# Reference - R for Statistical Learning - David Dalpiaz
# https://daviddalpiaz.github.io/r4sl/subset-selection.html


coef(regsubsets.out, 7)
coef(regsubsets.out, 10)

fit_bwd = regsubsets(median_house_value ~ (longitude + latitude + housing_median_age + total_rooms + population + median_income + ocean_proximity) ^ 2, data = housing_trn_data, nvmax = 10, method = "backward")
fit_bwd_sum = summary(fit_bwd)

fit_fwd = regsubsets(median_house_value ~ (longitude + latitude + housing_median_age + total_rooms + population + median_income + ocean_proximity) ^ 2, data = housing_trn_data, nvmax = 10, method = "forward")
...
#summary(best_exhaustive_model_1)
...
The second model is explore, `best_exhaustive_model_2`, has an Adjusted $R^2$ of $`r best_adjr2`$, which ties the Adjusted $R^2$ of `best_exhaustive_model_1`. To extract this model we are using the graph of the Adjusted $R^2$ to get the parameters from the second row from the top. There are similarities, but a couple differences here.
...
#summary(best_exhaustive_model_2)
...
The third model we'll explore, `best_exhaustive_model_3`, also scored an adjusted $R^2$ of $`r best_adjr2`$.
...
#summary(best_exhaustive_model_3)
