...
Note that the total bedrooms variable has *207* NA values. We will address this issue in our methods section.
...
The graph above displays a map of all the data points in this dataset, with longitude on the x-axis and latitude on the y-axis and each point being color codes to it's median home value level. We can see that there are two very popular locations, which are the homes on the coastline as well as in the Bay areas. Looking at the color gradient legend, we can see that the homes in these "clusters" are substiantially higher in value than in homes not on the coastline or near the north of the state. This tells us that location, or Ocean Proximity, will likely play a large role in predicting the median house value.
...
Looking at a barplot of the ocean proximity variable, we can see that the `ISLAND` factor is very low compared to every other factor in this variable. Let's look at the total count of each factor in this variable to get a better idea of the skewness in `ocean_proximity`.
...
We see that the `ISLAND` factor has five rows, where every other factor has over 2000 rows. Due to possible issues of fitting models later on, we believe that it is best to remove this factor from `ocean_proximity`.
...
Possibly create new variable of total bedrooms to households
...
We see that the model with the best AIC is `full_twoway_model`, with a score of ` r extractAIC(full_twoway_model)[2]`.

TODO: in addition to these 3 models, the model that Josh created initially should be integrated here. <- DONE
...
aic_and_bic_results = data.frame(
                                 "AIC" = c("Backward" = c("Two-Way" = extractAIC(back_twoway_mod_finish_aic)[2],
                                                          "Leap M1" = extractAIC(back_aic_mod_1)[2],
                                                          "Leap M2" = extractAIC(back_aic_mod_2)[2],
                                                          "Leap M3" = extractAIC(back_aic_mod_3)[2]
                                                          ),
                                           "Forward" =  c("Two-Way" = extractAIC(forward_twoway_mod_finish_aic)[2],
                                 ...
                                 "BIC" = c("Backward" = c("Two-Way" = extractAIC(back_twoway_mod_finish_bic)[2],
                                                          "Leap M1" = extractAIC(back_bic_mod_1)[2],
                                                          "Leap M2" = extractAIC(back_bic_mod_2)[2],
                                                          "Leap M3" = extractAIC(back_bic_mod_3)[2]
                                                          ),
                                           "Forward" =  c("Two-Way" = extractAIC(forward_twoway_mod_finish_bic)[2],
...
The models that we started with from leaps did not improve a whole lot, and neither did the model that we started with a full two-way interaction. In most cases they are the same, however we did find that `back_twoway_mod_finish_aic` slightly improved over `full_twoway_model`, which was the original model.
...
TODO: update the diagnostic function, as it is currently just the one from the homework (with change to limit to 5000 to make it work for this dataset) <- I will work on this ~Josh
...
TODO: discussion of the rad map and the hot spots by the coast and the Bay's <- done
TODO: is the "Near Bay" mean only San Fran or are there other Bay's? <- there are most likely other bays
