...
We evaluated our final model, the Backwards Two-Way Search model, on unseen test data using RMSE as well as average percent error.  We got an RMSE of $`r test_rmse`$, which compared to the LOOCV-RMSE we obtained on this model using our training data ($`r twoway_loocv_rmse`$), is a similiar result. Since these RMSEs are close in value, we can see that our model is not overfitting. For average percent error, we got a result of $`r test_perc_error`$%.
...
Overall, for pricing of homes, we believe this is a good amount of error to have as there are a lot of things that are taken into consideration when evaluating a home's value.

TODOs:

- moved these from the Results to the discussion - revisit them when I finish looking at the data.

```{r}
length(coef(back_twoway_mod_finish_aic))
length(coef(full_twoway_model))
names(coef(full_twoway_model))[which(!names(coef(full_twoway_model)) %in% names(coef(back_twoway_mod_finish_aic)))]
```

TODOs:

- Maybe there's something interesting in looking at the predictors that the back AIC took out of the full_twoway_model - one of this in the total_rooms:total_bedrooms which we noticed was highly correlated in the Intro.
...
op1[2] = 7
par(mar = op1)
plot(abs(test_predictions - test_actual), housing_tst_data$ocean_proximity,
     xlab = "Difference In Actual & Predicted House Prices",
     ylab = "",
     main = "Difference In Actual & Predicted House Prices vs Ocean Proximity",
     col = "dodgerblue", yaxt = "none")
axis(2, at = 1:5, labels = levels(housing_data$ocean_proximity), las = 1)
```

TODOs:

- Tie this to the original comments we made in the Introduction - that we suspected that `ocean_proximity` would play a key role but that there are some issues with the INLAND places such as near Yosemite and Tahoe.  That there are some outliers for which we were really off.
