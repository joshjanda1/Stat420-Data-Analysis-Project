...
Our final model, when evaluated on unseen test data, had an average error of $`r test_rmse`$.  This means that, on average, our model's predicted housing price will be $\pm$ $`r test_rmse`$ in comparison to the actual price.  This gives us an average percent error of $\pm$ $`r test_perc_error`$% between the actual and predicted prices.
...
Overall, our model was able to predict the prices of houses in California in 1990 within a $\pm$ $`r test_perc_error`$% error rate based on the California Housing Prices dataset.
