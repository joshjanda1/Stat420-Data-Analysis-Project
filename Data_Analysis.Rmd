...
```{r, echo = FALSE}
# this makes the numbers more human readable
# scipen controls when R kicks into scientific notation
...
The dataset contains $`r nrow(housing_data)`$ observations and 10 attributes (9 predictors and 1 response).  Below is a list of the variables with descriptions taken from the original Kaggle site given above.
...
The first test is the Anderson-Darling Test, which tests our normality assumption. The null hypothesis for this test is that our data *is* sampled from a normal distribution.  Note that we're using Anderson-Darling instead of Shapiro-Wilk because we are using a large dataset with $`r nrow(housing_data)`$ observations and Anderson-Darling performs better on larger datasets of this size.
...
We evaluated our final model, the Backwards Two-Way Search model, on unseen test data using RMSE as well as average percent error.  We got an RMSE of $`r test_rmse`$, which is similar to the LOOCV-RMSE we obtained on this model using our training data ($`r twoway_loocv_rmse`$). Since these RMSEs are close in value, we can see that our model is not overfitting. For average percent error, we got a result of $`r test_perc_error`$%.

# Discussion

Our final model, when evaluated on unseen test data, had an average error of $`r test_rmse`$, meaning that on average, our model will predict that a home will have a price $\pm$ $`r test_rmse`$ in comparison to the actual price.  This gives us an average percent error of $\pm$ $`r test_perc_error`$% difference between the actual and predicted prices.

While this is higher than we might have hoped for, we believe this error isn't too bad considering that the majority of the predictors were based on location.  As we noted in the Introduction, location - and more specifically `ocean_proximity` - does give us quite a lot of information about housing prices, but it can't tell the whole story.  We can see this in the range of the difference in actual and predicted prices for our model: 
...
plot(test_predictions - test_actual, housing_tst_data$ocean_proximity,
     xlab = "How Much Predicted Price Was Over / Under",
     ylab = "",
     main = "How Much Predicted Price Was Over / Under vs Ocean Proximity",
     col = "dodgerblue", yaxt = "none")
axis(2, at = 1:5, labels = levels(housing_tst_data$ocean_proximity), las = 1)

```

For the most part, the range of the difference in actual and predicted prices is consistent for each of the `ocean_proximity` levels (remembering that we removed the $ISLAND$ level to avoid skew due to its low representation in the dataset).  In general, we tend to underestimate the price just a little bit more frequently than overestimating, but the range is rather stable.  There do seem to be some outliers, however, when the `ocean_proximity` is equal to $INLAND$.  In particular, our model has underestimated the price of two houses by more than $800,000.  This is quite a jump from the rest of the errors.

In order to see if these data points line up with other potential issues we saw in the map in the Introduction, we plotted the difference between the predicted prices and the actual ones in the test data.  
...
It looks like those two troublesome data points are indeed located in one of the inland areas with more expensive prices - namely, near the Sacramento area.  It's possible that removing these two values could have helped us more accurately estimate the prices of inland houses.

Overall, our model did manage to predict the prices of houses in California in 1990 within a $\pm$ $`r test_perc_error`$% error rate based on the California Housing Prices dataset.
